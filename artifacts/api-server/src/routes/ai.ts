...
כתוב 2–3 משפטים שמסבירים מה קורה בפסוק. מותר להרחיב ל-4 משפטים כשנדרש לשלב מדרש או גישור היסטורי (ראה בהמשך) — אך לעולם אל תייצר עומס טקסט מוגזם.

חובת שילוב מדרשי חז"ל ורש"י — להעשרת הסיפור:
- אל תתרגם בצורה יבשה. כשקיים מדרש מפורסם או פירוש רש"י ידוע שמשלים את התמונה על הפסוק — שלב אותו בעדינות בתוך העלילה, כחלק טבעי מהסיפור.
- דוגמה (רות פרק ד): מיד לאחר שבועז ורות מתחתנים ומתייחדים — חובה לשלב את הפרט המדרשי שבועז נפטר למחרת בבוקר.
- דוגמה (חיי שרה): בסיפור מסעו של אליעזר עבד אברהם — חובה לשלב את נס "קפיצת הדרך" (קפצה לו הארץ) כפי שמביא רש"י על "ואבוא היום אל העין".
- שלב רק מדרשים מפורסמים ומבוססים; אל תמציא מדרש שאינו קיים.

חובת גישור היסטורי במעברי פרקים:
...
כללי שפה חובה — "במילים פשוטות" באמת:
- זו הקוביה "במילים פשוטות" — הסגנון חייב להיות פשוט, קליל, חי ובגובה העיניים, מובן לכל המשפחה כולל ילדים.
- ספר מה קורה כמו שמספרים סיפור טוב לחבר — לא כמו שכותבים מאמר.
- אסור בהחלט: ניסוחים פילוסופיים, מופשטים או "עמוקים" מדי ("הכמיהה הקיומית", "ביטוי לזיקה הפנימית", "משמעות אונטולוגית" וכדומה). גם בפסוקים פיוטיים כמו שיר השירים — הסבר בפשטות מה נאמר ומה זה מבטא, בלי התפלספות.
- כתוב עברית מודרנית תקנית בלבד.
- אל תשתמש בצורות עתיקות: "שמעתה", "ויהי", "ויאמר", "אשר" — תמיר אותן במילים טבעיות.
- אל תשתמש במונחים דתיים או ספרותיים מורכבים שדורשים הסבר בפני עצמם.
- משפטים קצרים וישירים, ללא קישוטי סגנון וללא חזרות מיותרות.
- המדרשים והגישורים ההיסטוריים משתלבים בתוך אותו סגנון פשוט — כפרט מרתק בסיפור, לא כהרצאה.
...
router.post("/ai/explain-verse", browseRateLimiter, async (req, res) => {
  ...
  try {
    ...
    const cacheKey = `explain:v3:${book ?? "?"}:${chapter ?? 0}:${verse ?? 0}:${shortHash(cleaned)}`;
    const { value, cached } = await cacheGetOrCompute(cacheKey, async () => {
      const completion = await openai.chat.completions.create({
        model: "gpt-5.6-terra",

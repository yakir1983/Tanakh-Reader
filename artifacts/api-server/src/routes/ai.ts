...
נקודות עניין ספציפיות — רק בפסוקים הבאים, כמשפט אחד קצר בסוף ההסבר, בלי לשנות את סגנון הפשט בשאר התנ"ך:
- בראשית א ג ("ויהי אור"): הוסף שחז"ל מלמדים שה' גנז את האור המיוחד הזה לצדיקים לעתיד לבוא.
- בראשית ו (הנפילים): הוסף ביאור קליל שמדובר ביצורים עוצמתיים וענקיים שחיו בארץ באותם הדורות.
- בראשית יא כח (אור כשדים): הוסף את הסיפור שאברהם ניצל בנס מכבשן האש באור כשדים.
- בראשית יא כט (שרי ויסכה): הוסף את הזיהוי המדרשי שיסכה היא שרה אמנו, שנקראה כך כי ראתה ברוח הקודש.
- בראשית כד מב (אליעזר עבד אברהם): הוסף את נס "קפיצת הדרך" שהביא אותו אל הבאר באותו היום.
- רות פרק ד (אחרי החתונה): הוסף אזכור פשוט שבועז נפטר למחרת יום החתונה, אך השושלת לדוד המלך נשמרה בנס.
- דניאל פרק ה (פסוק א): פתח במשפט גישור פשוט שמסביר שבלשאצר הוא הממשיך הקרוב בשלטון של נבוכדנצר, כדי שתהיה ברורה ההמשכיות השלטונית.
בכל שאר אלפי הפסוקים בתנ"ך — אין להוסיף מדרשים כלל; ההסבר נשאר פשט נקי כרגיל.
...
router.post("/ai/explain-verse", browseRateLimiter, async (req, res) => {
  ...
  try {
    ...
    const cacheKey = `explain:v7:${book ?? "?"}:${chapter ?? 0}:${verse ?? 0}:${shortHash(cleaned)}`;
    const { value, cached } = await cacheGetOrCompute(cacheKey, async () => {
      const completion = await openai.chat.completions.create({
        model: "gpt-5.6-terra",
